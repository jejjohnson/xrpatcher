...
1. **Tests** – `python -m pytest tests/ -q` must have 0 failures.
2. **Lint** – `python -m ruff check .` must report no issues.
3. **Format** – `python -m ruff format --check .` must report no files to reformat.
