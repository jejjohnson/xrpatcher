...
class XRDAPatcher:
    ...
    the input xr.DataArray should:
        - have coordinates
        - have for each dim of patch_dim (size(dim) - patch_dim(dim))
          divisible by stride(dim)
        (optional warning)
    """
    ...
    def __init__(
        self,
        da: xr.DataArray,
        patches: dict[str, int] | None = None,
        ...
        if check_full_scan:
            for dim in self.patches:
                if (self.da_dims[dim] - self.patches[dim]) % self.strides[dim] != 0:
                    ...
                    msg += (
                        f"{self.da_dims[dim]} - {self.patches[dim]}"
                        f" % {self.strides[dim]} "
                    )
    ...
    def reconstruct(
        ...
        dims_labels: list[str] | None = None,
        weight=None,
    ) -> xr.DataArray:
        """Reconstructs based on a list of patches, e.g. the output of a dataloader.
        ...
        Returns:
            rec_da (xr.DataArray): the reconstructed xr.DataArray that corresponds
        ...
        coords_labels: list[str] = [str(k) for k in coords[0].dims]
        ...
        msg = "Length of dim labels does not match length of dims."
        ...
        patches = {
            ikey: ivalue for ikey, ivalue in self.patches.items() if ikey in dims_labels
        }
        ...
        if weight is None:
            ...
            msg += (
                ...
                f" | Patches: {patch_values} | Dims: {item_shape}"
            )
        ...
        das = [
            ...
            for it, co in zip(items, coords, strict=True)
        ]
        ...
        # get new shape from
        new_shape = {
            idim: self.da[idim].shape[0] if idim in coords_labels else item_shape[i]
